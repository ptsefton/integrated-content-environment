...
var paqueteFactory = function(jQ){
    ...
    paquete.getSelectedNode = getSelectedNode;    
    ...
    function fixHashLocation(hash){
        ...
        //for IE which likes to replace our hash location with full url 
        var ourUrl = getRawLocationHref().split("#",1)[0];
        if(hash.substring(0, ourUrl.length)===ourUrl){
            hash = hash.substring(ourUrl.length);
        }
    ...
    function createNavigation(){
        ...
        if(paquete.navFormSettings.onNavFormDisplay){
            navForm = navigationFormFactory(paquete.navFormSettings,true);
            paquete.navFormSettings.onNavFormDisplay(navForm);
            return;
        }
    ...
    function checkNavigation(selectedNode){
        if(!paquete.navFormSettings.prevAlwaysActive){
        ...
        if(!paquete.navFormSettings.nextAlwaysActive){
        ...
        if(!paquete.navFormSettings.topAlwaysActive){
    ...
    function findNext(currentNode){
        ...
        if(nextNode.size() === 0){
            ...
            if(nextNode.size() === 0){ return; }//if no parent next sibling, end of list
        }
        return nextNode;
    }
    function goPrev(){
        ...
        if(prevNode && prevNode.size() !== 0) setHash(paquete.getThisNodeHash(prevNode));
    }
    function goNext(){
        currentNode = paquete.getCurrentDoc();
        nextNode =findNext(currentNode);
        if(nextNode  && nextNode.size() !==0) setHash(paquete.getThisNodeHash(nextNode));
    }
    function goTop(){
        paquete.contentScrollToTop();
    }
    var navigationFormFactory = function(settings,includeTop){
        ...
        settings.prevAlwaysActive =settings.prevAlwaysActive ||false;
        settings.nextAlwaysActive =settings.nextAlwaysActive ||false;
        settings.topAlwaysActive =settings.topAlwaysActive ||false;
        settings.onNavFormDisplay = settings.onNavFormDisplay || null;
        settings.includeTopButton = settings.includeTopButton || true;
        settings.openLeafClass = settings.openLeafClass|| "";
        settings.closedLeafClass = settings.closedLeafClass || "hide";
        ...
        if(includeTop && settings.includeTopButton){
    ...
    function getSelectedNode(hash){return jQ(paquete.tocSelector).find("a[href='"+hash+"']").parent("li");    }
    function updateTOC(){
        ...
        selectedNode = paquete.getSelectedNode("#"+hash);
        if(selectedNode.size() === 0){
            //fix for IE. IE replace the hash value with href+hash
            href = getLocationUrl();
            selectedNode = paquete.getSelectedNode(href+"#"+hash);
        ...
        jQ(paquete.tocSelector).find("li:has(ul)").removeClass(paquete.navFormSettings.openLeafClass);
        jQ(paquete.tocSelector).find("li:has(ul)").addClass(paquete.navFormSettings.closedLeafClass);
        selectedNode.parents("li").removeClass(paquete.navFormSettings.closedLeafClass);
        selectedNode.parents("li").addClass(paquete.navFormSettings.openLeafClass);
