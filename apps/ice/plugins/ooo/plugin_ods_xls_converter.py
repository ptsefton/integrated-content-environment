...
pluginName = "ice.ooo.OdsXlsConverter"
pluginDesc = "OdsXlsConverter"
...
class OdsConverter(object):
    ...
    def __init__(self, iceContext, fs, htmlTemplate):
        ...
        # .ods
        OdsConvert = self.iceContext.getPlugin("ice.render.odsConverter").pluginClass
        odsConvert = OdsConvert(self.iceContext)
        self.__odsRenderMethod = odsConvert.renderMethod
        
        # .xls
        XlsConvert = self.iceContext.getPlugin("ice.render.xlsConverter").pluginClass
        xlsConvert = XlsConvert(self.iceContext)
        self.__xlsRenderMethod = xlsConvert.renderMethod
        
        #.xlsx
        XlsxConvert = self.iceContext.getPlugin("ice.render.xlsxConverter").pluginClass
        xlsxConvert = XlsxConvert(self.iceContext)
        self.__xlsxRenderMethod = xlsxConvert.renderMethod
    ...
    def convert(self, file, toDir, options={}):
        ...
        if rep is not None:
            ...
            file = item.relPath
        if ext==".ods":
            convertedData = self.__render(file, self.__odsRenderMethod, self.__output, rep)
        elif ext==".xls":
            convertedData = self.__render(file, self.__xlsRenderMethod, self.__output, rep)
            tempOdsFile = convertedData.abspath("temp.ods")
            self.__fs.copy(tempOdsFile, self.__fs.join(path, name + ".ods"))
        elif ext==".xlsx":
            convertedData = self.__render(file, self.__xlsxRenderMethod, self.__output, rep)
            tempOdsFile = convertedData.abspath("temp.ods")
            self.__fs.copy(tempOdsFile, self.__fs.join(path, name + ".ods"))
        else:
            return ("Unsupported file extension '%s'." % ext, None, None)
        
        sourcelink = options.get("sourcelink", "off")
        if sourcelink == "off":
            if self.__fs.exists(convertedData.abspath(file)):
                self.__fs.delete(convertedData.abspath(file))
...
#        if bool(options.get("title", True)):
...
#        if bool(options.get("toc", True)):
        ...
        if self.__getOptionValue(options,"sourcelink" ) :
            sourceLink = "<a href='%s' title='View the original document'>Source</a>" % (name + ext)
            if ext == ".xls" or ext == ".xlsx":
                # include the generated ods for xls or xlsx
                sourceLink += " | <a href='%s' title='View the derived ODS'>Derived ODS</a>" % (name + ".ods")
