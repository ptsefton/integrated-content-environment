...
class OoObject(object):
    ...
    def loadDocument(self, fromFile, visible=False):
        ...
        self.setDocExt(os.path.splitext(fromFile)[1])
        inProps = self.__getInProperties()
        if visible:
            inProps = inProps[1:]
        try:
...
#                outputStream = OutputStream()       # Writes to sys.stdout
    ...
    def testBook(self, title=None):
        ...
        title="BuildBook"
        urlBookmarks = self.buildBook2(book, [two, one, four, three],"" ,title, visible=True)
        self.saveDocument("bookTestData/default1.book.odt", toExt=".odt")

    def testBook2(self, title=None):
        book = "bookTestData/default.book.odt"
        one = ("bookTestData/one.odt", "bookTestData/one.htm","", {"InsertPageBreak":False, "Page":"Odd", "Orientation":"Standard"})
        two = ("bookTestData/two.odt", "bookTestData/two.htm","", {"InsertPageBreak":False, "Page":"Odd", "Orientation":"Landscape"})
        three = ("bookTestData/three.odt", "bookTestData/three.odt","", {"InsertPageBreak":False})
        four = ("bookTestData/landscape.odt", "bookTestData/landscape.odt","", {"InsertPageBreak":False})
        title="BuildBook"
        urlBookmarks = self.buildBook2(book, [two, one, four, three],"" ,title, visible=True)
        self.saveDocument("bookTestData/default1.book.odt", toExt=".odt")
    ...
    def getPageStyle(self,doc):
        document = self.loadDocument(doc[0], False)
        self.gotoNextParagraph()
        pageStyle = self.cursor.PageStyleName
        return pageStyle
    ...
    def buildBook2(self, baseBookDocument, bookDocumentsPathUrl=[], baseUrl="http://localhost",
        ...
        #Get page styles for each document
        pageStyles = []
        for doc in bookDocumentsPathUrl:
            props = doc[3]
            if props.get("Orientation", "") != "":
                pageStyle = props.get("Orientation", "")
            else:
                pageStyle = self.getPageStyle(doc)
            pageStyles.append(pageStyle)
...
#            self.__cursor.PageNumberOffset = 1
        ...
        for doc, props in docs:
            ...
            if count==1:
                ...
                self.__cursor.BreakType = PAGE_BEFORE
                try:
                    self.__cursor.PageDescName=pageStyles[count-1]
                except:
                    self.__cursor.PageDescName="Standard"
                    #raise Exception("pageStyle='%s'" % pageStyle)
                self.__cursor.PageNumberOffset = 1
                #self.writeError("****1**** \n Doc : %s, PageStyle : %s, PageDesc : %s " % (doc, pageStyles[count-1], self.__cursor.PageDescName))
            else:
...
#                        bookmarkNames[bookmarkElement] = newName
    ...
    def reindexIng(self, bookDocument, visible=False):
        ...
        tocEndWithOdd = False
        for bookmarkElement in bookBookmarks.ElementNames:
            #self.writelnError ("BOOKMAKRELEMELENT: %s and pageNum: %s" % (bookmarkElement, pageNum))
            if bookmarkElement.find("_end_of_template_")>-1:
                self.gotoBookmark(bookmarkElement)
                pageNum = self.getPageNumberForCursor()
                isOdd = ((pageNum % 2)==1)
                tocPageNum = pageNum 
                #self.writelnError("pageNum: %s and isOdd: %s" % (pageNum, isOdd))
                if isOdd==True:
                    tocEndWithOdd = True
            if bookmarkElement.find("Start")>-1:
                ...
                isOdd = ((pageNum % 2)==1)
                if count == 1:
                    #to force the fix for the wrong end of TOC page 
#                   self.writelnError( "tocPageNum : %s, PageNum : %s, num diff ::%s" %(tocPageNum, pageNum,pageNum- tocPageNum))
#                   self.writelnError("Bookmark %s " % self.getPageNumberForBookmark("_end_of_template_"))
                    if self.getPageNumberForBookmark("_end_of_template_") != tocPageNum:
                        tocPageNum =self.getPageNumberForBookmark("_end_of_template_")
                        isTOCOdd = ((tocPageNum % 2)==1)
                ...
                if bookmarkElement.find("Odd")>-1:
                    ...
                    if count==1 and tocEndWithOdd and isOdd: #this is only to put blank page after TOC
                        #self.writelnError("tocPageNum: %s and tocEndWithOdd: %s" % (tocPageNum, tocEndWithOdd))
                        self.writelnError("insertPageBreak after TOC")
                        self.setViewCursorToCursor()
                        self.addBlankPageBefore()
                    if isOdd==False and count!=1:
                        self.setViewCursorToCursor()
...
#                    self.writelnError("count: %s and toc : %s , tocEndWithOdd: %s" % (count,tocPageNum, tocEndWithOdd))
                    if count==1 and not tocEndWithOdd and isOdd: #this is only to put blank page after TOC
                        #self.writelnError("count: %s and tocEndWithOdd: %s" % (count, tocEndWithOdd))
                        self.writelnError("insertPageBreak after TOC")
                        self.setViewCursorToCursor()
                        self.addBlankPageBefore()
                    if isOdd==True and count!=1:
                        self.writelnError( "PageNumber: %s for bookmarkElement:%s isEven=%s so insertPagebreak" % (pageNum, bookmarkElement, not(isOdd)))
                        self.setViewCursorToCursor()
                        self.addBlankPageBefore()
                    else:
                        self.writelnError( "PageNumber: %s for bookmarkElement:%s isEven=%s dont insertPageBreak" % (pageNum, bookmarkElement, not(isOdd)))
                    #self.deleteBookmark(bookmarkElement)                       
