...
var paqueteFactory = function(jQ){
    ...
    paquete.contentScrollToTop = function(){};
    ...
    paquete.updateContent = function(content) {
        ...
        if(window.location.hash) setTimeout(function(){window.location.hash=getLocationHash();paquete.updateTOC();}, 500);
    };
    ...
    function checkNavigation(selectedNode){
        ...
        if(selectedNode.prev("li").size() === 0 ){
        ...
        if(selectedNode.next("li").size() === 0 ){
    ...
    function goPrev(e){
        setHash(jQ("li.selected").prev("li").find("a").attr('href'));
    ...
    function goNext(e){
        setHash(jQ("li.selected").next("li").find("a").attr('href'));
        //console.log(jQ(e.target).parent().parent().next().children("a").attr("href"));
    ...
    function updateTOC(){
        ...
        selectedNode = jQ(paquete.tocSelector).find("li:has(a[href='"+"#"+hash+"'])");
