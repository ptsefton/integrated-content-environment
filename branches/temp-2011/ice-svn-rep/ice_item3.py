...
class IceItem(object):
    ...
    def __list(self, update=False):
        dirs, files = self.__fs.listDirsFiles(self.__absPath)
        l = [i+"/" for i in dirs]
        l.extend(files)
    ...
    def makeDirectory(self, message="mkdir", ignore=False):
        ...
        self.__fs.makeDirectory(self._absPath)
    ...
    def move(self, destItem):
        ...
        if self.isDirectory:
            ...
            self.__fs.move(absPath, absDestPath)    ##
        elif self.isFile:
            ...
            if self.__propAbsPath is not None:
                ...
                self.__fs.move(self.__propAbsPath, absDestPath)
        else:
            return
    ...
    def copy(self, destItem, annotation=False):
        ...
        if parentItem is not None and parentItem.exists==False:
            destItem.parentItem.makeDirectory()
        if self.isDirectory:
            self.__fs.copy(absPath, absDestPath)
        elif self.isFile:
            self.__fs.copy(absPath, absDestPath)
            destItem.__setup()
            if (self.__propAbsPath is not None) and (destItem.__propAbsPath is not None):
                absDestPath = "/.ice/".join(destItem.__absPath.rsplit("/", 1))
                self.__fs.copy(self.__propAbsPath, absDestPath)
        else:
            return
        destItem.__setup()
    ...
    def __getToBeRenderedList(self, itemList=None, force=False, skipBooks=True):
        ...
        if skipBooks==False:
            def extCmp(a, b):
                try:
                    pass
                    #print a.ext, b.ext, cmp(b.ext, a.ext)
                except: pass
